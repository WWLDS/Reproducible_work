...
As an example, let's suppose we have a dataframe that contains thousands of rows and hundreds of columns (this is common in the work we're doing!). You take a look at some of the columns and notice that two columns have different headers but seem to contain the same values. You write a bit of code to test this and it turns out it's true, all of them are the same! Although this may not seem like a classical example of an assumption, it's still something that you assume to hold true in the future, so you would note this down as an assumption. In this case you would do two things:

1) Create a notepad file in your project folder titled "assumptions.txt", note this assumption. Describe the assumption and the actions taken (in this case: removed column "x", kept column "y")

2) Comment out the test but do not delete (see Fig. 1).

3) Add a comment to this test line to explain the assumption in plain English.

To list a few other examples of assumptions: assuming the meaning of a column, assuming that two columns are collinear, assuming that if column_one == x, then column_two == y.
